...
"""Exact concurrent output checks, including shared-budget admission pressure."""
import argparse
import json
import os
...
def main():
    ...
    for index, (width,height,channels) in enumerate(cases):
        ...
        command = [binary / ("opj_compress"+SUFFIX), "-i", raw, "-o", source,
                   ...
                   "-mct", int(channels>=3), "-M", 63]
        if index%2:
            command.append("-I")
        execute(command, cpu)
        sources.append(source)
    def run(mode, env, callers):
        return json.loads(execute([binary / ("bench_workers"+SUFFIX), mode, references,
