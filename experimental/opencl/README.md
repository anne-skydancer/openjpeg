...
1. Extend the initial MQ decoder to RAW bypass, vertical causal mode and remaining
   supported coding semantics, using the new captured CPU reference boundary.
...
magnitude refinement, cleanup aggregation, context reset, terminated segments and
segmentation symbols. MQ states are generated from the pinned upstream table as
integer indices, retaining upstream notices. Tests verify the generated file is
current. RAW bypass, vertical causal mode, predictable termination checks, ROI
and HTJ2K are explicitly outside this first kernel's accepted domain.
...
ramp and random signals, three image sizes (including odd sizes), all eight
combinations of RESET/TERMALL/SEGSYM, both wavelet transforms and full/reduced
resolution decoding. Code blocks include partial blocks, 16x16 and up to 64x64.
...
Verified on RX 9070 XT / driver 3679.0: **2,112 blocks and 1,267,152 coefficients
match the CPU exactly**, including full 64x64 blocks. The corpus contains 144
codestreams decoded at two resolution settings. The selected 21-test suite passed;
after expanding block sizes and flag combinations, all three Tier-1 fixture,
validation and GPU tests passed again. A separate RelWithDebInfo build with
experiments disabled succeeded and contains no capture environment-variable
string. No throughput or end-to-end speedup has been measured.
...
experimental CPU decode. The GPU kernel reports a bad segmentation symbol as a
test failure; matching upstream's warning policy remains integration work.
